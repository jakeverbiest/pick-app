...
| Item | Type | Note |
|---|---|---|
| ~~**`MIN_CLEANUP_SECONDS` 20 → 120**~~ | code | **CLOSED 2026-09-08 — not a launch gate.** Not 20, and not going to 120: the value is **60** (`map.tsx:1907`), set in the Build 32 batch (`08ddb45`), and Jake confirmed 2026-09-08 that 60 is the standing default. This row's "Still 20 / RESTORE BEFORE LAUNCH" was stale by eight days. Note it does **not** guard the pocket-stop case — the guard is `elapsed < MIN && pickupCount === 0`. |
