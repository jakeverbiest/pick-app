...
<head>
  ...
  <style>
    ...
    /* Zoom control sits above the floating Start button, lowered closer to it.
       COUPLED to styles.adoptButton below, two ways:

       HEIGHT: two 44px buttons + borders measures 90px, so with the inner
       margin zeroed this occupies 96-186 and adoptButton's bottom must clear
       186. It currently sits at 202.

       Raised 84 -> 96 on 2026-09-09: at 84 the pill's lower edge crowded the
       attribution strip and the Start cleanup bar. adoptButton moved by the
       same 12 so the 16px gap between the two controls is unchanged — these
       two numbers travel together.

       HORIZONTAL CENTER: measured, not calculated — MapLibre nests the group
       inside a .maplibregl-ctrl wrapper carrying its own default margin:10px,
       so the outer margin alone does NOT position it. That margin is zeroed
       below; with it gone the group measures 46px outer, and margin-right 4
       puts its center 27px from the right edge. adoptButton is 44px at
       right 5, center also 27. Those numbers must move together.

       This is the bug the ORIGINAL comment here had: it claimed margin-right
       8 with a 38px control gave center 27, ignoring the same inner 10px, so
       the real center was ~37 and the two controls were never actually
       aligned. Verified against real MapLibre 4.7.1 in a browser before
       shipping; do not re-derive these by arithmetic alone.

       Styling deliberately mirrors the app rather than MapLibre's default:
       44px (not 38), radius 22 (not 12), and a 1.5px border with no shadow,
       matching styles.adoptButton and styles.toolOptionBtn. Before this, a
       44px bordered circle sat above a 38px shadowed rounded-rect and the two
       read as unrelated widgets rather than one control stack. */
    .maplibregl-ctrl-bottom-right { margin-bottom: 96px; margin-right: 4px; }
    ...
    .maplibregl-ctrl-group { border: 1.5px solid rgba(15,47,102,0.20) !important; box-shadow: none !important; border-radius: 22px !important; overflow: hidden; }
    .maplibregl-ctrl-group button { width: 44px !important; height: 44px !important; background: #fff !important; box-sizing: border-box; }
    .maplibregl-ctrl-group button + button { border-top: 1.5px solid rgba(15,47,102,0.20) !important; }
    .maplibregl-ctrl-group button:hover { background: #EEF3E6 !important; }
...
const styles = StyleSheet.create({
  ...
  adoptButton: {
    ...
    right: 5,
    // COUPLED to `.maplibregl-ctrl-bottom-right` in the map HTML above. That
    // control measures 90px tall (two 44px buttons plus borders) and sits 96px
    // up, so it occupies 96-186; this button must clear 186. Both were raised
    // 12px together on 2026-09-09 to get the pill off the attribution strip;
    // move one without the other and the 16px gap goes wrong.
    //
