...
import { osmNeighborhood, getHoodsInBounds, hoodLabelsNeeded, polygonStats, HoodShape } from '../../src/services/neighborhoods';
...
export default function MapScreen() {
  ...
  const loadHoodsInView = (b: [number, number, number, number]) => {
    ...
    const withLabels = hoodLabelsNeeded((b[0] + b[2]) / 2, (b[1] + b[3]) / 2);
...
<body>
  ...
  <script>
    ...
    function levelColor(daysOld) {
      if (daysOld === null || daysOld === undefined) return ['#C7CCC1', 2.5, 0.5]; // untouched — thin, light gray so it recedes and street names read through
