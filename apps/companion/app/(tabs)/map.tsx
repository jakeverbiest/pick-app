...
export default function MapScreen() {
  ...
  // True after goToCity() deliberately moves the map away from the real GPS
  // fix; false again only once recenter() explicitly snaps back. Exists
  // because the idle-recenter effect below (2026-07-14, one day BEFORE the
  // city switcher shipped) re-centers the map on every currentLocation change
  // while not mid-walk, with no awareness that goToCity ever existed — so
  // picking a distant city (e.g. Amsterdam from Brooklyn) got silently undone
  // by the next location tick, sometimes within seconds. A ref, not state:
  // nothing needs to re-render when this flips, only the effect's own guard
  // needs to read the latest value.
  const viewingOtherCityRef = useRef(false);
  // The last known real-location {city, neighborhood} from before goToCity()
  // overwrote currentArea, so recenter() can restore it INSTANTLY rather than
  // waiting on a fresh reverse-geocode. goToCity() already does this trick for
  // itself (sets the city label the moment you pick one, then refines it),
  // but recenter() never got the matching half — invisible while the map
  // itself wasn't moving (2026-09-10's setView bug), now a visible stale-label
  // beat now that jumpTo is instant: reported by Jake as "recenter snaps the
  // map back but retains the city I had just been in."
  const homeAreaRef = useRef<{ city: string; neighborhood: string } | null>(null);
  ...
  // Set when activateHood's coverage fetch fails outright (e.g. every
  // Overpass mirror down) rather than just being slow. Distinct from
  // slowLoadBanner: this is a dead end, not "still working" — never render
  // it as a false "complete"/0% result. { name, ring } so Retry can re-run
  // the same activation.
  const [activationError, setActivationError] = useState<{ name: string; ring: [number, number][] } | null>(null);
  ...
  // never yanks the map during a walk. Also stands down while browsing a
  // different city via goToCity() — see viewingOtherCityRef's declaration.
  useEffect(() => {
    if (!currentLocation || isListening || !mapReady || viewingOtherCityRef.current) return;
    webviewRef.current?.injectJavaScript(
  ...
  const activateHood = async (name: string, ring: [number, number][], reveal: boolean) => {
    ...
    // A genuine fetch failure (e.g. every Overpass mirror down) must never
    // reach finish() — finish() treats a 0-segment result as legitimate data
    // (0% freshPct, 0 toGo), and toGo === 0 elsewhere renders as "complete!".
    // That's correct for "nothing left to clean" but wrong for "we never got
    // any data" — so failures are routed to a dedicated error state instead.
    const fail = () => {
      if (activationTokenRef.current !== token || !activeLevelRef.current) return;
      setActivating(null);
      setSlowLoadBanner(null);
      setLiveNowCount(null);
      setActivationError({ name, ring });
    };

    const coveragePromise = getCoverageForRing(ring);
    const timedOut: unique symbol = Symbol('activate-timeout') as any;
    let raced: RenderSegment[] | typeof timedOut;
    try {
      ...
      // coveragePromise itself rejected (all mirrors + tiled fallback failed)
      // before the timeout even had a chance to fire.
      fail();
      return;
    }
  ...
  // Clears the level-mode STATE (React side only — no map/WebView call, no
  // recentering). Shared by exitLevel() (the explicit back button) and
  // recenter() (which must do the same teardown when tapped from inside a
  // level — see recenter()'s own comment for why this exists: reported by
  // Jake as "dimming still there" after recentering from inside a
  // neighborhood, because recenter() moved the camera but never cleared
  // activeLevel, leaving the veil and its stats locked on the neighborhood
  // you'd tapped into).
  const teardownLevelState = () => {
  ...
  const exitLevel = () => {
    ...
    //
    // jumpTo, not setView — setView is Leaflet's API and doesn't exist on a
    // MapLibre GL map (confirmed against maplibre-gl 4.7.1: throws "map.setView
    // is not a function"). The 2026-09-08 port replaced the WebView's OWN
    // Leaflet init but missed the three places outside it — here, goToCity()
    // and recenter() — that inject this call as a JS string, so all three
    // silently did nothing for two days: the surrounding try/catch swallowed
    // the error with no logging. Center order is also [lon, lat] here, not
    // [lat, lon] — MapLibre's convention, matching window.updateLocation's own
    // easeTo call inside the WebView.
    webviewRef.current?.injectJavaScript(`
      if (window.exitLevel) { window.exitLevel(); }
      ${typeof la === 'number' ? `try { map.jumpTo({ center: [${lo}, ${la}], zoom: 15 }); } catch (e) { console.error('exitLevel recenter failed:', e.message); }` : ''}
      true;
    `);
...
Generated by Pick App - Share this with the development team
  ...
  // Marks viewingOtherCityRef so the idle-recenter effect above stands down —
  // see that ref's declaration for why this is needed at all.
  //
  // jumpTo, not setView, and [lon, lat] not [lat, lon] — see exitLevel's
  // comment above for why (setView doesn't exist on MapLibre GL; this was
  // the actual cause of "map doesn't change when I enter a new city", not
  // the recenter-effect conflict this function's viewingOtherCityRef fix
  // addressed the same day — that fix was real but this bug meant it never
  // got the chance to matter, since the map never moved in the first place).
  const goToCity = (c: { label: string; lat: number; lon: number; zoom: number }) => {
    setCityPickerOpen(false);
    setSelectedHood(null);
    // Stash the real-location area BEFORE it gets overwritten below, so
    // recenter() can restore it instantly instead of waiting on a fresh
    // geocode — see homeAreaRef's declaration. Guarded on !viewingOtherCityRef
    // so hopping between several other cities in a row (Amsterdam, then
    // Hickory) only ever captures the true home value, never a
    // previously-browsed one.
    if (!viewingOtherCityRef.current) homeAreaRef.current = currentArea;
    viewingOtherCityRef.current = true;
    ...
    webviewRef.current?.injectJavaScript(
      `try { map.jumpTo({ center: [${c.lon}, ${c.lat}], zoom: ${c.zoom} }); } catch (e) { console.error('goToCity failed:', e.message); } true;`
    );
    refreshArea(c.lat, c.lon);
    loadHoodsInView([c.lat - 0.012, c.lon - 0.016, c.lat + 0.012, c.lon + 0.016]);
  };

  /** Zoom via the RN control stack. Goes through window.zoomMap in the map
   *  HTML rather than a MapLibre control, so the zoom buttons live in the same
   *  coordinate space as the tools button — see styles.zoomStack. */
  const zoomBy = (delta: number) => {
    webviewRef.current?.injectJavaScript(
      `if (window.zoomMap) { window.zoomMap(${delta}); } true;`
    );
  };

  // Snap back to where you actually are. The only place viewingOtherCityRef
  // clears — until this runs, the idle-recenter effect leaves a
  // goToCity()-browsed map alone rather than fighting it on every GPS tick.
  // jumpTo/[lon, lat], not setView — see goToCity's comment; this button has
  // been silently doing nothing since the 2026-09-08 MapLibre port too.
  //
  // Also tears down level mode if you're inside one — reported by Jake: tap
  // into a neighborhood, hit Recenter, and the dimmed veil + its % stat
  // stayed locked on that neighborhood even though the camera and header
  // correctly moved to the real location. The tools menu's Recenter option
  // is deliberately reachable from inside a level (unlike the header/city
  // switcher, which hide there), so this is a real, reachable path, not an
  // edge case — see teardownLevelState()'s own comment.
  const recenter = () => {
    const loc = currentLocation;
    if (!loc) return;
    if (activeLevel) teardownLevelState();
    viewingOtherCityRef.current = false;
    // Restore the real-location label INSTANTLY rather than waiting on
    // refreshArea's network geocode — see homeAreaRef's declaration. Without
    // this the map snaps home immediately but the header keeps showing
    // whatever city you'd been browsing until the geocode resolves, which
    // reads as "recenter half-worked" even though the map itself is correct.
    // refreshArea still runs below and will refine/confirm this shortly,
    // exactly as it already does after goToCity's own optimistic set.
    if (homeAreaRef.current) {
      setCurrentArea(homeAreaRef.current);
      homeAreaRef.current = null;
    }
    webviewRef.current?.injectJavaScript(`
      if (window.exitLevel) { window.exitLevel(); }
      try { map.jumpTo({ center: [${loc.lon}, ${loc.lat}], zoom: 16 }); } catch (e) { console.error('recenter failed:', e.message); }
      true;
    `);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Shown when the coverage fetch fails outright (e.g. every Overpass
          mirror down) rather than just being slow — never silently fall
          through to a false "complete"/0% result. Retry re-runs the same
          activation. */}
      {activationError && !activating && (
        <View style={[styles.slowLoadBanner, { top: insets.top + 8 }]}>
          <Text style={styles.slowLoadBannerText} numberOfLines={2}>
            Couldn't load street detail for {activationError.name} — try again
          </Text>
          <TouchableOpacity
            onPress={() => {
              const { name, ring } = activationError;
              setActivationError(null);
              activateHood(name, ring, true);
            }}
            accessibilityLabel="Retry loading street detail"
          >
            <Text style={styles.slowLoadBannerDismiss}>Retry</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setActivationError(null)} accessibilityLabel="Dismiss error">
            <Text style={styles.slowLoadBannerDismiss}>✕</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {cityRequestCard && !activating && (
        <View style={[styles.cityRequestCard, { top: insets.top + (slowLoadBanner || activationError ? 56 : 8) }]}>
...
{/* Real Map - Expands when cleaning */}
      <View style={[styles.mapContainer, isListening && styles.mapContainerExpanded]}>
        ...
        {/* Zoom. Rendered here rather than as a maplibregl.NavigationControl so
            it shares a coordinate space with the tools button above it — see
            the note where addControl used to be. Deliberately OUTSIDE the
            !isListening guard below: the MapLibre control stayed available
            during a walk and removing that would be a regression. */}
        <View style={styles.zoomStack}>
          ...
          >
            <Icon name="minus" size={20} color={C.primary} />
          </TouchableOpacity>
        </View>
        ...
        {!isListening && !activating && (
          <>
            ...
            >
              <Icon name={toolsOpen ? 'close' : 'layers'} size={22} color={(toolsOpen || !!impactWindow) ? '#fff' : C.primary} />
            </TouchableOpacity>
          </>
        )}
...
<head>
  ...
  <style>
    ...
    /* The zoom control is gone (see the note where addControl used to be) —
       only the attribution stays in the WebView's corners now. */
    .maplibregl-ctrl-bottom-left { margin-bottom: 0; margin-left: 0; }
    .maplibregl-ctrl-attrib { font-size: 10px; padding: 1px 5px; background: rgba(255,255,255,0.7) !important; }
    /* Neighborhood name labels are a real symbol layer now (see hood-labels),
       not divIcon markers — so the basemap's own collision detection applies and
       our names no longer overprint its place labels. */
    .seg-popup .maplibregl-popup-content { background: #1B2E1A; color: #fff; border-radius: 10px; box-shadow: 0 4px 14px rgba(27,46,26,0.28); margin: 0; padding: 8px 12px; font-size: 13px; font-weight: 600; line-height: 1.2; }
    .seg-popup .maplibregl-popup-tip { border-top-color: #1B2E1A; border-bottom-color: #1B2E1A; }
...
<body>
  ...
  <script>
    /* ─────────────────────────────────────────────────────────────────────
     * The Map tab, on MapLibre GL + CARTO's Positron VECTOR style.
     * Ported from Leaflet 2026-09-08 — step 3 of
     * docs/VECTOR_BASEMAP_MIGRATION_SCOPE.md §5, and the last of the four maps.
     *
     * THE ONE STRUCTURAL CHANGE WORTH KNOWING. Leaflet built this out of
     * FeatureGroups that were cleared, refilled and re-ordered at runtime
     * ('clearLayers', 'bringToBack', custom panes at z-index 340/350). MapLibre
     * renders a single ordered layer array, so instead the ENTIRE STACK IS
     * DECLARED ONCE, bottom to top, at load — and every render function only
     * calls setData on a source. Nothing re-orders itself, which removes a
     * whole class of "why is this underneath now" bug.
     *
     * Consequences to keep in mind when editing:
     *  - To add a layer, put it in the stack below at the right height. Do not
     *    add it later and try to move it.
     *  - The two Leaflet panes are now just positions in that stack:
     *    level streets sit below the veil, the veil below everything else,
     *    exactly as z-index 340/350 did.
     *  - 'line-dasharray' is NOT data-driven in MapLibre (unlike color/width),
     *    so anything dashed needs its own layer with a filter. That is why
     *    segments and level streets each have a solid layer and a dashed one.
     *
     * COORDINATE ORDER: this file speaks [lat, lon] everywhere, because that is
     * what React Native sends and what Firestore stores. MapLibre and GeoJSON
     * speak [lon, lat]. 'll()' flips, and it is the only place flipping happens.
     * ───────────────────────────────────────────────────────────────────── */
${MAPLIBRE_ANCHOR_FN}

    var ll = function (p) { return [p[1], p[0]]; };
    var lls = function (a) { return (a || []).map(ll); };
    var EMPTY = { type: 'FeatureCollection', features: [] };
    var feat = function (geom, props) { return { type: 'Feature', properties: props || {}, geometry: geom }; };
    var lineF = function (coords, props) { return feat({ type: 'LineString', coordinates: lls(coords) }, props); };
    var ptF = function (p, props) { return feat({ type: 'Point', coordinates: ll(p) }, props); };

    /** Metres between two [lat, lon] points — replaces Leaflet's map.distance. */
    ...
    function bboxOf(lonLatPts) {
      return lonLatPts.reduce(function (acc, c) {
    ...
    // No maplibregl.NavigationControl here on purpose. It lived inside the
    // WebView while the map tools button is a React Native sibling, so the two
    // sat in DIFFERENT coordinate systems — the WebView's bottom edge is not
    // adoptButton's bottom edge, and the offset between them is a runtime
    // product of the tab bar and safe area. Three attempts to align them by
    // arithmetic each measured correct in isolation and shipped visibly
    // overlapping (2026-09-09). Zoom is now RN buttons in the same stack as
    // the tools button, so spacing is exact by construction. See styles.zoomStack.
      ...
      try {
        ...
        if (window.ReactNativeWebView) {
          window.ReactNativeWebView.postMessage(JSON.stringify({
            type: 'moveend', lat: c.lat, lon: c.lng, zoom: map.getZoom(),
            b: [bb.getSouth(), bb.getWest(), bb.getNorth(), bb.getEast()]
          }));
        }
      } catch (e) {}
    });
    ...
    function segFresh(daysOld) { return [freshColor(daysOld), 0.85 * tailFade(daysOld)]; }
    ...
    // ── State the render functions mutate, then push with setData ───────────
    var levelLocked = false;
    var routeCoords = [];
    var cleanedById = {};
    var todoById = {};
    var CLEANED_CAP = 4000; // hard ceiling so a marathon session can't grow unbounded
    var hoodRings = {};     // name → ring, so a redraw can re-tint the selected one
    ...
    var src = function (id) { return map.getSource(id); };

    /* Sources do not exist until MapLibre's 'load' fires, and the React Native
     * side starts calling render functions as soon as the WebView reports
     * ready — which is EARLIER. Under Leaflet that was fine: layers existed the
     * moment the map object did. Here an unbuffered setData would silently
     * no-op and the data would simply be gone.
     *
     * Caught 2026-09-08 by driving all 28 window.* functions against a real
     * style: parks, adoptions, historical routes and pickups all came back
     * empty while the map looked healthy. Buffer instead, and flush on load. */
    var pendingData = {};
    ...
    map.on('load', function () {
      ...
      [
        'parks', 'hoods', 'hist', 'histDots', 'segTodo', 'segCleaned', 'hoodCov',
        'level', 'veil', 'impact', 'impactDots', 'need', 'adoptHi', 'adopted',
        'route', 'pickups', 'user'
      ].forEach(function (id) { map.addSource(id, { type: 'geojson', data: EMPTY }); });

      var add = function (spec) { map.addLayer(spec, anchor); };

      // ── THE STACK, bottom to top. Order here IS the z-order. ──────────────
      add({ id: 'parks-fill', type: 'fill', source: 'parks',
            paint: { 'fill-color': ['get', 'color'], 'fill-opacity': ['get', 'fill'] } });
      add({ id: 'parks-line', type: 'line', source: 'parks',
      ...
      add({ id: 'hood-fill', type: 'fill', source: 'hoods',
            paint: { 'fill-color': ['case', ['get', 'sel'], '#0F2F66', '#5A6B8C'],
                     'fill-opacity': ['case', ['get', 'sel'], 0.06, 0.0] } });
      add({ id: 'hood-line', type: 'line', source: 'hoods',
            layout: { 'line-join': 'round' },
            paint: { 'line-color': ['case', ['get', 'sel'], '#0F2F66', '#5A6B8C'],
                     'line-width': ['case', ['get', 'sel'], 2.5, 1.5],
                     'line-opacity': ['case', ['get', 'sel'], 0.85, 0.6] } });

      add({ id: 'hist-line', type: 'line', source: 'hist',
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': ['get', 'color'], 'line-width': 12, 'line-opacity': ['get', 'op'] } });
      add({ id: 'hist-dot', type: 'circle', source: 'histDots',
            paint: { 'circle-radius': 8, 'circle-color': ['get', 'color'], 'circle-opacity': ['get', 'op'] } });

      // Dashed and solid must be separate layers: line-dasharray takes no
      // data-driven expression, unlike line-color and line-width.
      add({ id: 'seg-todo', type: 'line', source: 'segTodo',
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': NEVER_COLOR, 'line-width': 4, 'line-opacity': 0.4, 'line-dasharray': [2, 9] } });
      add({ id: 'seg-cleaned', type: 'line', source: 'segCleaned',
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': ['get', 'color'], 'line-width': ['get', 'w'], 'line-opacity': ['get', 'op'] } });

      add({ id: 'hoodcov-line', type: 'line', source: 'hoodCov',
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': ['get', 'color'], 'line-width': 3, 'line-opacity': ['get', 'op'] } });

      // Level streets sit BELOW the veil — the old levelPane (z-index 340) —
      // so a street crossing the neighborhood boundary is masked outside it.
      add({ id: 'level-dashed', type: 'line', source: 'level',
            filter: ['==', ['get', 'untouched'], true],
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': ['get', 'color'], 'line-width': ['get', 'w'],
                     'line-opacity': ['get', 'op'], 'line-dasharray': [2, 9] } });
      add({ id: 'level-solid', type: 'line', source: 'level',
            filter: ['==', ['get', 'untouched'], false],
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': ['get', 'color'], 'line-width': ['get', 'w'], 'line-opacity': ['get', 'op'] } });

      // The veil — the old maskPane (z-index 350). A world rectangle with the
      // neighborhood punched out, so the city outside fades to soft cream.
      add({ id: 'veil-fill', type: 'fill', source: 'veil',
            paint: { 'fill-color': '#FFFFFF', 'fill-opacity': 0.55 } });
      add({ id: 'veil-line', type: 'line', source: 'veil',
            layout: { 'line-join': 'round' },
            paint: { 'line-color': '#0F2F66', 'line-width': 2.5, 'line-opacity': 0.95 } });

      // Invisible fat tap target for level streets, ABOVE the veil so taps still
      // land. Replaces Leaflet's duplicate weight-18 hit polylines.
      add({ id: 'level-hit', type: 'line', source: 'level',
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': '#000', 'line-width': 18, 'line-opacity': 0 } });

      add({ id: 'impact-line', type: 'line', source: 'impact',
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': '#4B7A54', 'line-width': 5, 'line-opacity': 0.7 } });
      add({ id: 'impact-dot', type: 'circle', source: 'impactDots',
            paint: { 'circle-radius': 5, 'circle-color': '#4B7A54', 'circle-opacity': 0.95,
                     'circle-stroke-color': '#ffffff', 'circle-stroke-width': 1 } });

      add({ id: 'need-dot', type: 'circle', source: 'need',
            paint: { 'circle-radius': 11, 'circle-color': ['get', 'color'], 'circle-opacity': 0.6,
                     'circle-stroke-color': '#ffffff', 'circle-stroke-width': 1 } });

      add({ id: 'adopt-hi', type: 'line', source: 'adoptHi',
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': '#4B7A54', 'line-width': 8, 'line-opacity': 0.95 } });

      // Navy dots for adopted blocks. Deliberately NOT a colored line: the street
      // underneath is already colored by freshness (green→red), and covering that
      // would hide whether an adopted block still needs a cleanup. Navy appears
      // nowhere in that gradient, so a dot reads as "you adopted this", not
      // "here's its status".
      add({ id: 'adopted-dot', type: 'circle', source: 'adopted',
            paint: { 'circle-radius': 6, 'circle-color': '#0F2F66',
                     'circle-stroke-color': '#FFFFFF', 'circle-stroke-width': 2 } });

      add({ id: 'route-line', type: 'line', source: 'route',
            layout: { 'line-cap': 'round', 'line-join': 'round' },
            paint: { 'line-color': '#4B7A54', 'line-width': 14, 'line-opacity': 0.55 } });
      add({ id: 'pickup-dot', type: 'circle', source: 'pickups',
            paint: { 'circle-radius': 6, 'circle-color': '#FF3B30', 'circle-opacity': 0.7,
                     'circle-stroke-color': '#fff', 'circle-stroke-width': 1 } });
      add({ id: 'user-dot', type: 'circle', source: 'user',
            paint: { 'circle-radius': 8, 'circle-color': '#4B7A54', 'circle-opacity': 0.8,
                     'circle-stroke-color': '#fff', 'circle-stroke-width': 2 } });

      // Neighborhood names go ABOVE the basemap's labels (no anchor), as a real
      // symbol layer rather than divIcon markers — so MapLibre's own collision
      // detection applies and our names never overprint the basemap's.
      map.addLayer({
        id: 'hood-labels', type: 'symbol', source: 'hoods',
        layout: {
          'text-field': ['get', 'name'], 'text-size': 12, 'text-letter-spacing': 0.1,
          'text-transform': 'uppercase', 'text-font': ['Open Sans Semibold'],
          'symbol-placement': 'point', 'text-allow-overlap': false, 'visibility': 'none'
        },
        paint: { 'text-color': '#7C8A74', 'text-halo-color': '#F5F5F0', 'text-halo-width': 1.6 }
      });

      // Seed the default dot first, so a real position buffered before load
      // overwrites it rather than the other way round.
      setData('user', { type: 'FeatureCollection', features: [ptF([40.7128, -74.0060])] });
      Object.keys(pendingData).forEach(function (id) {
    ...
    // Every render function below is safe to call before the style finishes
    // loading: it stores into the state above and, once ready, pushes. flushAll
    // replays that state on load so nothing sent during the fetch is dropped.
    function flushAll() {
      if (!ready) return;
      redrawTodo(); redrawCleaned(); redrawRouteSrc(); redrawHoods(); redrawLevel();
    }
    ...
    map.on('click', function (e) {
      if (window.__adoptMode && window.ReactNativeWebView) {
        window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'adoptTap', lat: e.lngLat.lat, lon: e.lngLat.lng }));
      }
    });
    window.highlightBlock = function (coords) {
      setData('adoptHi', { type: 'FeatureCollection', features: coords && coords.length ? [lineF(coords)] : [] });
    };
    window.clearBlockHighlight = function () { setData('adoptHi', EMPTY); };

    window.renderMyAdoptions = function (items) {
      var fs = (items || []).filter(function (a) {
        return typeof a.lat === 'number' && typeof a.lon === 'number';
      }).map(function (a) { return ptF([a.lat, a.lon], { label: a.label || 'Adopted block' }); });
      setData('adopted', { type: 'FeatureCollection', features: fs });
    };
    map.on('click', 'adopted-dot', function (e) {
      var f = e.features && e.features[0];
      if (!f) return;
      new maplibregl.Popup({ closeButton: false, className: 'seg-popup', offset: 10 })
        .setLngLat(e.lngLat)
        .setHTML('<b>' + f.properties.label + '</b><br>You adopted this — you\\'ll get a nudge if it goes stale.')
        .addTo(map);
    });

    // ── Live walk: user dot, route, pickups ─────────────────────────────────
    /** Zoom entry point for the RN control stack (see styles.zoomStack).
     *  Positive delta zooms in. Wrapped because an inject that throws leaves
     *  no trace on the RN side. */
    window.zoomMap = function (delta) {
      try { if (delta > 0) { map.zoomIn(); } else { map.zoomOut(); } } catch (e) {}
    };

    window.updateLocation = function (lat, lon) {
      setData('user', { type: 'FeatureCollection', features: [ptF([lat, lon])] });
      ...
      var c = map.getCenter();
      if (distanceM([c.lat, c.lng], [lat, lon]) > 30) {
        map.easeTo({ center: [lon, lat], duration: 300 });
      }
    };

    function redrawRouteSrc() {
      setData('route', { type: 'FeatureCollection', features: routeCoords.length > 1 ? [lineF(routeCoords)] : [] });
    }
    ...
    window.addRoutePoint = function (lat, lon) { routeCoords.push([lat, lon]); redrawRouteSrc(); };

    var pickupPts = [];
    window.addPickup = function (lat, lon) {
      pickupPts.push(ptF([lat, lon]));
      setData('pickups', { type: 'FeatureCollection', features: pickupPts });
    };

    window.setInitialLocation = function (lat, lon) { map.jumpTo({ center: [lon, lat], zoom: 16 }); };

    // ── "Need" and "My impact" overlays ─────────────────────────────────────
    window.renderNeed = function (tiles) {
      setData('need', { type: 'FeatureCollection',
        features: (tiles || []).map(function (t) { return ptF([t.lat, t.lon], { color: t.color }); }) });
    };
    window.clearNeed = function () { setData('need', EMPTY); };

    window.renderImpact = function (routes, pickups) {
      var rs = (routes || []).filter(function (r) { return r && r.length > 1; });
      setData('impact', { type: 'FeatureCollection', features: rs.map(function (r) { return lineF(r); }) });
      setData('impactDots', { type: 'FeatureCollection', features: (pickups || []).map(function (p) { return ptF(p); }) });
      try {
        if (rs.length) {
          var pts = [];
          rs.forEach(function (r) { r.forEach(function (p) { pts.push(ll(p)); }); });
          var b = bboxOf(pts);
          map.fitBounds([[b[0], b[1]], [b[2], b[3]]], { padding: 40, duration: 0 });
        }
      } catch (e) {}
    };
    window.clearImpact = function () { setData('impact', EMPTY); setData('impactDots', EMPTY); };

    // ── Historical walks ────────────────────────────────────────────────────
    window.addHistoricalRoutes = function (cleanups) {
      var lines = [], dots = [];
      (cleanups || []).forEach(function (cleanup) {
        var daysOld = (Date.now() - cleanup.timestamp) / (1000 * 60 * 60 * 24);
        var color = freshColor(daysOld);
        var opacity = 0.55 * tailFade(daysOld);
        var drew = false;
        if (cleanup.route_points) {
          try {
            var coords = typeof cleanup.route_points === 'string' ? JSON.parse(cleanup.route_points) : cleanup.route_points;
            if (Array.isArray(coords) && coords.length > 1) {
              lines.push(lineF(coords, { color: color, op: opacity }));
              drew = true;
            }
          } catch (e) {}
        }
        if (!drew) dots.push(ptF([cleanup.location_lat, cleanup.location_lon], { color: color, op: opacity }));
      });
      setData('hist', { type: 'FeatureCollection', features: lines });
      setData('histDots', { type: 'FeatureCollection', features: dots });
    };
    ...
    // ── Street-segment coverage (shared across ALL users) ───────────────────
    // Gray dashes = never cleaned; green→red = freshness since last clean.
    // Cleaned segments ACCUMULATE across fetches (keyed by id) so your progress
    // persists when you pan; the gray "todo" dashes are replaced each fetch.
    function redrawCleaned() {
      ...
      setData('segCleaned', { type: 'FeatureCollection', features: ids.map(function (id) {
    ...
    function redrawTodo() {
      if (!ready) return;
      setData('segTodo', { type: 'FeatureCollection', features: Object.keys(todoById).map(function (id) {
        return lineF(todoById[id].coords);
      }) });
    }
    ...
    // Live in-walk recolor: flip finished blocks to bright fresh-green without
    // disturbing the rest of the coverage history.
    window.markSegmentsClean = function (ids) {
      if (!ids || !ids.length) return;
      var promoted = false, changed = false;
      ids.forEach(function (id) {
        if (cleanedById[id]) { cleanedById[id].daysOld = 0; changed = true; }
        else if (todoById[id]) {
          cleanedById[id] = { coords: todoById[id].coords, daysOld: 0 };
          delete todoById[id]; promoted = true; changed = true;
        }
      });
      if (!changed) return;
    ...
    window.clearSegments = function () {
      cleanedById = {}; todoById = {};
      setData('segTodo', EMPTY); setData('segCleaned', EMPTY);
    };

    // ── Neighborhood outlines ───────────────────────────────────────────────
    function redrawHoods() {
      if (!ready) return;
      var fs = Object.keys(hoodRings).map(function (name) {
        var ring = lls(hoodRings[name]);
        return feat({ type: 'Polygon', coordinates: [ring.concat([ring[0]])] },
                    { name: name, sel: name === selectedHoodName });
      });
      setData('hoods', { type: 'FeatureCollection', features: fs });
    }
    window.renderNeighborhoods = function (list, withLabels) {
      if (!list || !list.length) return;
      list.forEach(function (h) { if (!hoodRings[h.name]) hoodRings[h.name] = h.ring; });
      redrawHoods();
      // Our own names only where the basemap doesn't already print them (Atlanta).
      pendingLabels = !!withLabels;
      setVisible(['hood-labels'], pendingLabels);
    };
    map.on('click', 'hood-fill', function (e) {
      if (window.__adoptMode) return; // adopt mode: tap selects a block, not a hood
      var f = e.features && e.features[0];
      if (f && window.ReactNativeWebView) {
        window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'hoodTap', name: f.properties.name }));
      }
    });
    window.highlightNeighborhood = function (name) { selectedHoodName = name; redrawHoods(); };

    // ── Level mode: the whole hood as a bounded "level" you fill in ─────────
    var WORLD_RING = [[-85, -180], [-85, 180], [85, 180], [85, -180]];
    var OUTSIDE_LAYERS = ['hist-line', 'hist-dot', 'hood-fill', 'hood-line', 'hood-labels', 'parks-fill', 'parks-line'];
    var hoodLabelsWanted = false;
    ...
    function drawSpotlight(ring) {
      ...
      // (a light scrim, not a dark one). GeoJSON rings must close explicitly.
      var outer = lls(WORLD_RING); outer = outer.concat([outer[0]]);
      var inner = lls(ring); inner = inner.concat([inner[0]]);
      setData('veil', { type: 'FeatureCollection',
        features: [feat({ type: 'Polygon', coordinates: [outer, inner] })] });
    }
    ...
    window.enterLevel = function (ring) {
      ...
      hoodLabelsWanted = pendingLabels;
      setData('segTodo', EMPTY); setData('segCleaned', EMPTY);
      setVisible(OUTSIDE_LAYERS, false);
      levelFeatures = []; levelIndexById = {}; redrawLevel();
      drawSpotlight(ring);
      try {
        var b = bboxOf(lls(ring));
        map.fitBounds([[b[0], b[1]], [b[2], b[3]]], {
          // clear the level header up top; tight margins elsewhere → fill the screen
          padding: { top: 92, bottom: 28, left: 12, right: 12 },
    ...
    // Adopt lives in the popup (no separate adopt mode): tap a street →
    ...
    map.on('click', 'level-hit', function (e) {
      var f = e.features && e.features[0];
      if (!f) return;
      var days = f.properties.hasDays ? f.properties.daysOld : null;
      new maplibregl.Popup({ closeButton: false, className: 'seg-popup', offset: 8 })
        .setLngLat(e.lngLat)
        .setHTML(
          '<span class="seg-popup-text">' + freshnessText(days) + '</span>' +
          '<br><a class="seg-popup-adopt" onclick="window.__adoptFromPopup(' + e.lngLat.lat + ',' + e.lngLat.lng + ')">Adopt this block</a>'
        )
        .addTo(map);
    });

    function levelFeature(s) {
      var untouched = (s.daysOld === null || s.daysOld === undefined);
      var c = levelColor(s.daysOld);
      return lineF(s.coords, {
        id: s.id || '', untouched: untouched, hasDays: !untouched,
        daysOld: untouched ? 0 : s.daysOld, color: c[0], w: c[1], op: c[2]
      });
    }
    function redrawLevel() {
      if (!ready) return;
      setData('level', { type: 'FeatureCollection', features: levelFeatures });
    }
    ...
    // thousands of segments; building them in one synchronous pass froze the UI
    ...
    window.renderLevel = function (list) {
      ...
      function step() {
        ...
        for (; i < end; i++) {
          var s = list[i];
          if (s.id) levelIndexById[s.id] = levelFeatures.length;
          levelFeatures.push(levelFeature(s));
        }
        redrawLevel();
    ...
    window.markLevelClean = function (id) {
      var idx = levelIndexById[id];
      if (idx === undefined) return;
      var f = levelFeatures[idx];
      f.properties.color = '#2FB457';
      f.properties.w = 5;
      f.properties.op = 0.95;
      f.properties.untouched = false;
      f.properties.hasDays = true;
      f.properties.daysOld = 0; // tapping it now says "Cleaned today"
      redrawLevel();
    };
    ...
    window.exitLevel = function () {
      ...
      levelFeatures = []; levelIndexById = {}; redrawLevel();
      setData('veil', EMPTY);
      setVisible(OUTSIDE_LAYERS, true);
      setVisible(['hood-labels'], hoodLabelsWanted);
      ...
      redrawTodo(); redrawCleaned();
    };

    // ── Parks ───────────────────────────────────────────────────────────────
    window.renderParks = function (parks) {
      setData('parks', { type: 'FeatureCollection', features: (parks || []).map(function (park) {
        var color, fill;
        ...
        var ring = lls(park.polygon); ring = ring.concat([ring[0]]);
        return feat({ type: 'Polygon', coordinates: [ring] }, { color: color, fill: fill });
      }) });
    };

    // ── Neighborhood coverage underlay ──────────────────────────────────────
    window.showNeighborhoodCoverage = function (cleanups) {
      if (!cleanups || cleanups.length === 0) return;
      var fs = [];
      cleanups.forEach(function (cleanup) {
        var daysOld = (Date.now() - cleanup.timestamp) / (1000 * 60 * 60 * 24);
        if (!cleanup.route_points) return;
        try {
          var coords = typeof cleanup.route_points === 'string' ? JSON.parse(cleanup.route_points) : cleanup.route_points;
          if (Array.isArray(coords) && coords.length > 1) {
            fs.push(lineF(coords, { color: freshColor(daysOld), op: 0.6 * tailFade(daysOld) }));
          }
        } catch (e) {}
      });
      setData('hoodCov', { type: 'FeatureCollection', features: fs });
    };
    window.clearNeighborhoodCoverage = function () { setData('hoodCov', EMPTY); };
    ...
    window.setCoverageVisible = function (visible) {
      setVisible(['seg-cleaned', 'seg-todo', 'hood-fill', 'hood-line', 'hist-line', 'hist-dot'], visible);
    };
  </script>
</body>
</html>`,
...
const styles = StyleSheet.create({
  ...
  adoptButton: {
    position: 'absolute',
    // Horizontally centered over the zoom control below it: that control
    // measures 46px outer at margin-right 4 (with MapLibre's inner 10px
    // wrapper margin zeroed — see the CSS note above) → center = 4 + 23 =
    // 27px from the right edge. This 44px button matches: right = 27 − 22 = 5.
    right: 5,
    // Sits directly above styles.zoomStack, which is 90px tall at bottom 96
    // and so occupies 96-186. Both are React Native siblings in one
    // coordinate space now, so this is simple arithmetic that stays true —
    // unlike the previous arrangement, where the zoom control lived in the
    // WebView and no amount of arithmetic aligned them.
    //
    // Was 170, which left 10px and rendered as a visible ~8px overlap on a
    // 390pt device. The original 170 was tuned when the control was LEAFLET's
    // with its margin zeroed; the 2026-09-08 MapLibre port set margin-bottom
    // to 84px and lifted it into this button's space. 190 restores real
    // clearance AND matches the `bottom: 190 + 58 * n` baseline the tool
    // options above already use, so the whole column is now on one rhythm.
    //
    // Do not lower the control instead: Start cleanup's top edge is only
    // ~63px above the map's bottom, so 84px is already near-minimal for it.
    bottom: 202,
    width: 44,
    height: 44,
    borderRadius: 22,
  ...
  // Zoom stack. Same width and right offset as adoptButton, so the two centre
  // on the same axis by construction (44 wide at right 5 → centre 27 from the
  // right edge, both). 43 + 1.5 divider + 43 + 3 border = 90.5 tall, sitting
  // at bottom 96 → occupies 96-186, leaving a 16px gap to adoptButton at 202.
  zoomStack: {
    position: 'absolute',
    right: 5,
    bottom: 96,
  ...
  zoomBtn: { height: 43, alignItems: 'center', justifyContent: 'center' },
  zoomDivider: { height: 1.5, backgroundColor: C.border },
