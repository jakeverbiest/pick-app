...
Generated by Pick App - Share this with the development team
  ...
  // Cities you can jump to directly (the fine-neighborhood cities). Centered on
  // the dense core so the hood outlines are right there when you land.
  const CITIES = [
    { id: 'nyc', label: 'New York City', lat: 40.6782, lon: -73.9442, zoom: 14 },
    { id: 'atl', label: 'Atlanta', lat: 33.749, lon: -84.388, zoom: 14 },
  ];

  // Toggle the "recent cleanups" overlay (was the top-right route button).
  const toggleNeighborhoodOverlay = async () => {
  ...
  const goToCity = (c: { lat: number; lon: number; zoom: number }) => {
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
          <View style={{ flex: 1, minWidth: 0 }}>
            ...
            {selectedHood ? (
              ...
              <Text style={styles.coverageText}>Tap the name to switch city · tap a neighborhood for progress</Text>
            )}
          </View>
          <View style={styles.headerControls}>
            ...
            <TouchableOpacity
              ...
              onPress={toggleNeighborhoodOverlay}
              accessibilityLabel="Toggle recent cleanups"
            >
              <Icon name="route" size={20} color={showNeighborhood ? '#fff' : COLORS.sage} />
            </TouchableOpacity>
            <TouchableOpacity
      ...
      <View style={[styles.mapContainer, isListening && styles.mapContainerExpanded]}>
        {/* Recenter on my location — sits just above the zoom control */}
        {!isListening && !activeLevel && !activating && currentLocation && (
          <TouchableOpacity
  ...
  <script>
    ...
    window.renderNeighborhoods = function(list) {
      ...
      list.forEach(function(h) {
        ...
        // No custom name label — the CARTO basemap already prints neighborhood
        // names (the soft grayscale labels), so we let those stand on their own.
      ...
      <Modal visible={cityPickerOpen} transparent animationType="fade" onRequestClose={() => setCityPickerOpen(false)}>
        <TouchableOpacity style={styles.cityBackdrop} activeOpacity={1} onPress={() => setCityPickerOpen(false)}>
          <View style={styles.citySheet}>
            <Text style={styles.citySheetTitle}>Jump to a city</Text>
            {CITIES.map((c) => (
              <TouchableOpacity key={c.id} style={styles.cityRow} onPress={() => goToCity(c)} activeOpacity={0.7}>
                <Icon name="pin" size={18} color={COLORS.sage} sw={1.8} />
                <Text style={styles.cityRowText}>{c.label}</Text>
              </TouchableOpacity>
            ))}
            {currentLocation && (
              <TouchableOpacity style={styles.cityRow} onPress={() => { setCityPickerOpen(false); recenter(); }} activeOpacity={0.7}>
                <Icon name="target" size={18} color={COLORS.sage} sw={1.8} />
                <Text style={styles.cityRowText}>My location</Text>
              </TouchableOpacity>
            )}
          </View>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  cityRowText: { fontSize: 16, fontWeight: '600', color: COLORS.darkSage },
  mapButton: {
    width: 40,
    height: 40,
    borderRadius: 20,
