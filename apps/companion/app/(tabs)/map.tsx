...
<head>
  ...
  <style>
    ...
    /* Neighborhood name labels — prominent, no box, halo for legibility over the map.
       Rendered as divIcon markers (reliable in WebView, unlike permanent tooltips). */
    ...
    .hood-label-text { display: inline-block; transform: translate(-50%, -50%);
      color: #1B2E1A; font-weight: 800; font-size: 13px; letter-spacing: -0.2px;
  ...
  <script>
    ...
    window.renderNeighborhoods = function(list) {
      ...
      list.forEach(function(h) {
        ...
        // the primary wayfinding cue, so it stays visible without a tap. A
        // divIcon marker renders reliably in the WebView (permanent tooltips
        // don't always auto-open here).
        try {
          var lc = poly.getBounds().getCenter();
          var safe = String(h.name).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
          var label = L.marker(lc, {
            interactive: false, keyboard: false,
            icon: L.divIcon({ className: 'hood-label', html: '<span class="hood-label-text">' + safe + '</span>', iconSize: [0, 0] })
          }).addTo(boundaryGroup);
          hoodLabels[h.name] = label;
        } catch (e) {}
