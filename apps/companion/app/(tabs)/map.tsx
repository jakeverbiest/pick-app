  ...
  const loadHistoricalCleanups = async () => {
    try {
      ...
      if (webviewRef.current && cleanups.length > 0) {
        ...
        webviewRef.current.injectJavaScript(`
          if (window.addHistoricalRoutes) { window.addHistoricalRoutes(${cleanupJson}); }
          true;
        `);
      }
    } catch (error) {
              ...
              onPress={async () => {
                ...
                if (newState) {
                  ...
                  try {
                    ...
                    if (webviewRef.current && cleanups.length > 0) {
                      webviewRef.current.injectJavaScript(`
                        if (window.showNeighborhoodCoverage) { window.showNeighborhoodCoverage(${JSON.stringify(cleanups)}); }
                        true;
                      `);
                    }
                  } catch (error) {
