...
<head>
  ...
  <style>
    ...
    /* Zoom control sits above the floating Start button, lowered closer to it.
       COUPLED to styles.adoptButton below: this control is 76px tall, so at
       84px up it occupies 84-160 and adoptButton's bottom must clear 160.
       Changing this number without changing that one puts the map tools "+"
       back on top of the zoom pill, which is what shipped 2026-09-08. */
    .maplibregl-ctrl-bottom-right { margin-bottom: 84px; margin-right: 8px; }
    ...
    .maplibregl-ctrl-group { border: none !important; box-shadow: 0 2px 8px rgba(27,46,26,0.18) !important; border-radius: 12px !important; overflow: hidden; }
    .maplibregl-ctrl-group button { width: 38px !important; height: 38px !important; background: #fff !important; }
...
const styles = StyleSheet.create({
  ...
  adoptButton: {
    position: 'absolute',
    // Horizontally centered over the zoom control below it: that control sits
    // 8px from the right at 38px wide → center = 8 + 19 = 27px from the right
    // edge. This 44px button matches: right = 27 − 22 = 5.
    right: 5,
    // COUPLED to `.maplibregl-ctrl-bottom-right`'s margin-bottom in the map
    // HTML above. That control is 76px tall (two 38px buttons) and sits 84px
    // up, so it occupies 84-160; this button must clear 160.
    //
